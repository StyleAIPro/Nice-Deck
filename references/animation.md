...
普通滚轮在滚动模式下仍用于浏览页面；只有按住 `Ctrl/Cmd` 才缩放并阻止滚动。倍率写入 `localStorage`，刷新后恢复；非 100% 时顶部导航 glass bar 向右液态展开一个无底色的四角框，框内显示当前百分比，点击恢复 100% 后收回。用户点击右上角播放 / 滚动任一模式按钮时也必须先恢复 100%，但初始化模式不得清除刷新恢复的倍率。内容缩放由 deck 自己接管，不使用浏览器页面缩放，避免自动适配反向抵消。点击推进只在放映模式生效，且点在按钮 / 链接 / iframe 等交互元素上时交给元素自己处理、不推进节拍。进页时点击计数 `level = 0`，只显示没挂动画的元素；每前进一拍 `level + 1`。
