...
点击推进只在放映模式生效，按钮 / 链接 / iframe 等交互元素自行处理；抓手锁定时单击空白也不得推进。进页 `level = 0`，每前进一拍 `level + 1`。公共运行时修改必须写入 `scripts/runtime_migrations.py`，同步三套模板并在 `upgrade_deck.py` 添加独立 probe，不能只改某一份 deck。
