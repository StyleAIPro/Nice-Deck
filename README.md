...
把一批反馈交给 Agent 后，如果在右侧 CLI 中按独立的 `Esc` 中断当前执行，编辑器会同步把本批状态改为“已取消”，未完成成员进入“批次 N · 未完成”，可选择仅重试剩余任务或明确合并到下一批；长期 Codex / Claude Code / OpenCode PTY 与会话身份继续保留。方向键等以 ESC 开头的终端控制序列不会被误判为批次中断。
