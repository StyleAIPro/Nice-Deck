...
**目标：** 从 77 页《训练Infra课程-独立版 (2).html》做减法，产出 24 页「页型画廊」模板 deck，连同文档与工具打包成自包含 Claude Code skill `aico-ppt`。
...
**规格：** `docs/superpowers/specs/2026-07-06-aico-ppt-skill-design.md`
...
- 每次改模板 deck 后必跑：`python3 dc-slide-deck/edit-bundle.py "class-1/aico-ppt/assets/training-deck.html"`（即 `eb.verify`，断言 slide-fit/sections/nav 三者一致且 nav i: 连续）。
...
**文件：**
- 创建：`class-1/aico-ppt/assets/training-deck.html`
...
mkdir -p aico-ppt/assets aico-ppt/references aico-ppt/scripts/html2pptx aico-ppt/scripts/verify
```
...
DST = os.path.join(ROOT, 'aico-ppt/assets/training-deck.html')
SEP = '\n\n    '
...
`node "$ROOT/scratchpad/measure_overflow.mjs" "$ROOT/aico-ppt/assets/training-deck.html" 封面 版式·卡片网格 结语页`
注意：该旧脚本第 10 行有 `length > 60` 的等待条件，对 24 页会 falsy——先临时把该行 `60` 改 `20`（此脚本在 scratchpad，可改）。预期三页都能找到、section overflow Y=0。

- [ ] **步骤 5：checkpoint**

```bash
cp "$ROOT/aico-ppt/assets/training-deck.html" "$SCRATCH/_ckpt_task1_rebuilt.html"
```
...
**文件：**
- 修改：`class-1/aico-ppt/assets/training-deck.html`
...
运行：`python3 "$SCRATCH/tpl_task3_prune.py"`；然后 `ls -lh aico-ppt/assets/training-deck.html`。
...
**文件：**
- 创建：`class-1/aico-ppt/scripts/apply_bg.py`
- 创建：`class-1/aico-ppt/references/branding.md`
...
```bash
cd "$ROOT"
cp dc-slide-deck/edit-bundle.py aico-ppt/scripts/
cp dc-slide-deck/react.umd.js dc-slide-deck/react-dom.umd.js aico-ppt/scripts/
cp dc-slide-deck/html2pptx/{convert.py,convert.sh,build_pptx.py} aico-ppt/scripts/html2pptx/
cp dc-slide-deck/html2pptx/shoot.mjs aico-ppt/scripts/html2pptx/
chmod +x aico-ppt/scripts/html2pptx/convert.sh
```

- [ ] **步骤 6.5：shoot.mjs 去机器绑定** —— 第 8 行 import 改成与 6.1 相同的 `PLAYWRIGHT_CORE` env 方式（convert.py/build_pptx.py 无绝对路径，无需改）。
- [ ] **步骤 6.6：html2pptx 全流程实测** —— `python3 aico-ppt/scripts/html2pptx/convert.py aico-ppt/assets/training-deck.html "$SCRATCH/training-deck.pptx"`（Windows 用 `py -3`）。预期：23 页 deck 出 ≥30 张（「动画·layer切换」4 标签 +3、「动画·混合链」5 标签 +4；精确张数以 shoot.mjs 输出为准并记录到 editing-guide.md）；打开 pptx 抽查 layer 展开页齐全、build 页全显。
...
- [ ] **步骤 1：完整性** —— `python3 aico-ppt/scripts/edit-bundle.py aico-ppt/assets/training-deck.html`：slide-fit=sections=nav=23、i: 连续、chapters 5 条 start={1,5,14,18,21}。
...
- [ ] **步骤 6：`SKILL.md`** —— frontmatter：`name: aico-ppt`；`description: Use when creating a new Huawei-red-brand 1920×1080 single-file HTML slide deck from the bundled template (华为风红色品牌网页 PPT), editing such a deck (pages/animations/branding), or converting it to PPTX via the bundled html2pptx — template gallery, click/arrow-key present mode, offline single file.`（措辞与 dc-slide-deck 区分：本 skill 管「从模板新建」，dc 管「维护训练Infra课程」。）正文 ≤120 行：① 这是什么（一段）；② 快速上手 5 步工作流（cp 模板→照 template-pages.md 改页→增删页用 edit-bundle→verify 工具→convert.py）；③ 设计铁律 6 条摘要（各一行，指向 references 细读）；④ 文件导航表；⑤ 预期性能一句话（体积/首开秒数，任务 7 实测值）。
...
- [ ] **步骤 1：安装** —— `cp -R "$ROOT/aico-ppt" ~/.claude/skills/aico-ppt`。
- [ ] **步骤 2：模拟新用户全流程**（在 `$SCRATCH/e2e/` 下，全程只用 skill 内文件与文档所写命令）：
  1. `cp ~/.claude/skills/aico-ppt/assets/training-deck.html demo.html`
...
- [ ] **步骤 3：收尾** —— e2e 产物留在 `$SCRATCH`；向用户报告：skill 位置（项目内 + ~/.claude/skills）、模板 deck 页数/体积、验证结论、分发方式（打包 aico-ppt/ 目录）。
